...
EMAIL_URL = reverse("taxi:contact")


class PublicTests(TestCase):
    def test_login_required(self):
        manufacturers = self.client.get(MANUFACTURER_LIST_URL)
        cars = self.client.get(CAR_LIST_URL)
        drivers = self.client.get(DRIVER_LIST_URL)
        contact = self.client.get(EMAIL_URL)

        self.assertNotEqual(manufacturers.status_code, 200)
        self.assertNotEqual(cars.status_code, 200)
        self.assertNotEqual(drivers.status_code, 200)
        self.assertNotEqual(contact.status_code, 200)
...
class PrivateContactTest(TestCase):
    def setUp(self):
        self.user = get_user_model().objects.create_user("test", "password123")
        self.client.force_login(self.user)

    def test_retrieve_contact_form(self):

        res = self.client.get(EMAIL_URL)

        self.assertEqual(res.status_code, 200)
        self.assertTemplateUsed(res, "email.html")
